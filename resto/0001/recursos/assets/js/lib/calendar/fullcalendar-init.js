var base_url = "http://pxsistemas.com/resto/0001/" //MODIFICAR SEGUN EL LINK DEL PROYECTO
var URLactual = window.location;
var carpeta = '/resto/0001/'
...
!function($) 
{
    ...
    {   
        ...
        form.append("<div class='input-group'><input class='form-control' type=text value='" + calEvent.title + "' /><span class='input-group-btn'><button type='submit' class='btn btn-success waves-effect waves-light'><i class='fa fa-check'></i> Guardar</button></span></div>");
            ...
            $this.$modal.find('.delete-event').show().end().find('.save-event').hide().end().find('.modal-body').empty().prepend(form).end().find('.delete-event').unbind('click').on("click", function () 
            {
                ...
                $.ajax({
                    type: "POST",
                    url: "http://pxsistemas.com/resto/0001/planificaciones/eliminarevento", // url que me recibe los datos
                    dataType: 'json',
                    data: datosEnviados,
            ...
            $this.$modal.find('form').on('submit', function () 
            {
                ...
                $.ajax({
                    type: "POST",
                    url: "http://pxsistemas.com/resto/0001/planificaciones/editarevento", // url que me recibe los datos
                    dataType: 'json',
                    data: datosEnviados,
                    success: function (msg) {
    ...
    CalendarApp.prototype.onSelect = function (start, end, allDay) {
        ...
        //// ---------------- ESTE CÓDIGO ES PARA CREAR UN NUEVO EVENTO ------------------------------////
                    ...
                    $.ajax({
                        type: "POST",
                        url: "http://pxsistemas.com/resto/0001/planificaciones/crearevento", // url que me recibe los datos
                        dataType: 'json',
                        data: datosEnviados,
                        success: function (msg) 
                        {
    ...
    CalendarApp.prototype.init = function() 
    {
        ...
        $.ajax({
            ...
            dataType: "json",
            url: "http://pxsistemas.com/resto/0001/planificaciones/obtener_eventos", // url que me recibe los datos
            data: datosEnviados,
