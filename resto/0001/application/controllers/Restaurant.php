...
//// JORNADA 	| VISTA INICIAR JORNADA
	public function iniciarjornada()
	{
		...
		{	
			...
			$datos = array('Estado_jornada'=> $datos_jornada["Estado"], 'Jornada_id' => $datos_jornada["Id"]);
	...
	public function obtener_resumen_jornadas()
    {
		...
		foreach ($array_jornadas as $jornada) 
		{
			...
			// CONSULTANDO INGRESOS COMANDAS
				...
				{
					$Total_ing_comandas = $comanda["Valor_cuenta"] - $comanda["Valor_descuento"] + $comanda["Valor_adicional"];
				}
			...
			// CONSULTANDO INGRESOS DELIVERY
				...
				$total_delivery = $query->num_rows();

				$Total_ing_delivery = 0;

				foreach ($array_delivery_jornada as $comanda) 
				{
					$Total_ing_delivery = $comanda["Valor_cuenta"] - $comanda["Valor_descuento"];
				}
			...
			// MOVIMIENTOS DE CAJA
				...
				{
					$Result_caja = $caja["Valor_ingreso"] - $caja["Valor_egreso"];
				}
...
//// COMANDAS 	| CARGAR O EDITAR COMANDAS
	public function crear_comanda()
    {
		...
		if(isset($this->datosObtenidos->comandaData->Id))
        {
            $Id = $this->datosObtenidos->comandaData->Id;
        }
        else
        {
            $Id = NULL;
		...
		$data = array(
					...
					'Fecha' => $fecha,
					'Hora_llegada' => $Hora,
					'Modo_pago' => $this->datosObtenidos->comandaData->Cliente_referente,
...
//// COMANDAS 	| OBTENER lISTADO DE COMANDAS ABIERTAS
	public function obtener_listado_comandas_abiertas()
    {
		...
		foreach ($array_comandas as $comanda) 
		{
			...
			//// Armado de array
			$comanda = array('Datos_comanda' => $comanda, 'Datos_items'=> $result, 'Total_items' => $total_todos, 'Cant_entregados'=> $cant_entregados, 'Cant_pendientes'=> $total_pendientes, 'Alerta' => $Alerta);
...
//// DELIVERY	| OBTENER LISTADO ABIERTOS
	public function obtener_listado_deliverys_abiertos()
    {
		...
		foreach ($array_deliverys as $delivery) 
		{
			...
			//// Armado de array
			$Data_delivery = array('Datos_delivery' => $delivery, 'Datos_items'=> $result, 'Total_items' => $total, 'Alerta' => $Alerta);
...
//// DELIVERY 	| CARGAR O EDITAR
	public function crear_delivery()
    {
		...
		$data = array(
                    'Jornada_id' =>			$Jornada_id,
					'Repartidor_id' => 		$this->datosObtenidos->deliveryData->Repartidor_id,
					'Nombre_cliente' => 	$this->datosObtenidos->deliveryData->Nombre_cliente,
					'Direccion' => 			$this->datosObtenidos->deliveryData->Direccion,
					'Telefono' => 			$this->datosObtenidos->deliveryData->Telefono,
					'Modo_pago' => 			$this->datosObtenidos->deliveryData->Modo_pago,
					'Estado' => 			0,
                );
