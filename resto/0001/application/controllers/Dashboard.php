...
{ 
	...
	{   
        ...
        {
            if ($this->session->userdata('Rol_id')== 5 or $this->session->userdata('Rol_id')== 1) /// si es delivery o colaborador no puede acceder
            {
    ...
    /// info Minuto a minuto
        public function infoTimeline()
        {
            ...
            if($this->datosObtenidos->Peticion == "Next") { $inicio =  $Pagina_actual + $cantidadItems; }
            elseif($this->datosObtenidos->Peticion == "Prev") { $inicio =  $Pagina_actual - $cantidadItems; }
